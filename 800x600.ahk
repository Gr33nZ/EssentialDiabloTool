...
;=======================================================================================================================;
;															;
;Adjusting Vars 											|(800x600)	;
InvPos1X := 515	        ;Position Top Left in Inventory X (Coord) 					|515		;
InvPos1Y := 311 	;Position Top Left in Inventory Y (Coord)					|311		;
InvSpace := 28		;Spacing between each Inventory Field  (pixel)					|28		;
InvStartX := 487	;Start on next Row (InvPos1X - InvSpace)					|487 		;
Sle := 1		;Sleep Timer (ms)   				         			|1		;
FuInv := 1		;Inventory Position of your Mouse (Fix)	        				|1		;
CubeFillX := 385	;Cube Fill X (Coord) 								|385		;
CubeFillY := 470	;Cube Fill Y (Coord)								|470		;
CubeTransX := 100 	;Cube Transmute x (Coord) 							|100		;
CubeTransY := 455 	;Cube Transmute Y (Coord)							|455		;
CubeRemoveX := 135	;Position Top Mid in Cube X (Coord) 						|135		;
CubeRemoveY := 210	;Position Top Mid in Cube Y (Coord)						|210		;
CubeSle := 500		;Sleep between each Action (ms) 						|500		;
CubeAccept := 2500	;Sleep Timer for Cube (ms | Fix)						|2500		;
CubeSpace := 56	        ;For Fields that have a height of 2 (InvSpace * 2) 				|56		;
DisPosX := 390		;Discard Position X (Coord) 							|390		;
DisPosY := 285		;Discard Position Y (Coord)							|285		;
;															;
;=======================================================================================================================;
